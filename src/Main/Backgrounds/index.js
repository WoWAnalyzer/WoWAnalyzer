import bosses from 'common/bosses';

// Want to contribute a screenshot? Add your own and make a pull request!
// Screenshots with no boss names are preferred. Screenshots with a player character is only permitted if the player is barely visible.

export default {
  // Tomb of Sargeras
  [bosses.TombOfSargeras.GOROTH]: [
    'http://wow.zamimg.com/uploads/screenshots/normal/608510.jpg', // with boss
    'http://wow.zamimg.com/uploads/screenshots/normal/608512-tomb-of-sargeras.jpg', // with boss
    'http://8gmwp015fo-flywheel.netdna-ssl.com/wp-content/uploads/sites/9/2017/05/Q8D9BQGQJTJG14956550272861.jpg', // boss artwork
  ],
  [bosses.TombOfSargeras.DEMONIC_INQUISITION]: [
    'http://wow.zamimg.com/uploads/screenshots/normal/608580-tomb-of-sargeras.jpg', // empty
    'https://pbs.twimg.com/media/C7tfAHZXgAEsw88.jpg', // with bosses
    'http://wow.zamimg.com/uploads/screenshots/normal/636604-atrigan.jpg', // bosses, close up
  ],
  [bosses.TombOfSargeras.HARJATAN]: [
    'http://wow.zamimg.com/uploads/screenshots/normal/608518.jpg', // with boss
    'https://bnetcmsus-a.akamaihd.net/cms/content_entry_media/SOPTO2Y0N1M91495655027092.jpg', // boss artwork
  ],
  [bosses.TombOfSargeras.MISTRESS_SASSZINE]: [
    'http://wow.zamimg.com/uploads/screenshots/normal/608534-tomb-of-sargeras.jpg', // empty floor view
  ],
  [bosses.TombOfSargeras.SISTERS_OF_THE_MOON]: [
    'http://wow.zamimg.com/uploads/screenshots/normal/608545-tomb-of-sargeras.jpg', // empty, looking at boss room from nearby
    'http://wow.zamimg.com/uploads/screenshots/normal/608540-tomb-of-sargeras.jpg', // empty, looking at boss room from far
  ],
  [bosses.TombOfSargeras.THE_DESOLATE_HOST]: [
    'http://wow.zamimg.com/uploads/screenshots/normal/608584.jpg', // empty
  ],
  [bosses.TombOfSargeras.MAIDEN_OF_VIGILANCE]: [
    'http://wow.zamimg.com/uploads/guide/seo/5215.jpg', // boss close
    'https://i.ytimg.com/vi/RACdQg4tnZI/hq720.jpg', // with boss + name
  ],
  [bosses.TombOfSargeras.FALLEN_AVATAR]: [
    'http://i.imgur.com/gxmLtFh.jpg', // empty boss room
    'http://i.imgur.com/NThnlW8.jpg', // empty after boss (green water)
    'http://i.imgur.com/zhIxvnd.jpg', // empty (looking at side)
  ],
  [bosses.TombOfSargeras.KILJAEDEN]: [
    'https://i.ytimg.com/vi/1ZHebAubjms/maxresdefault.jpg', // with boss + name
    'http://wow.zamimg.com/uploads/blog/images/11420.jpg', // empty
    'http://wow.zamimg.com/uploads/blog/images/11416.jpg', // side view with character slightly visible
    'http://wow.zamimg.com/uploads/guide/seo/5230.jpg', // boss close up
  ],
  // Antorus, the Burning Throne
  [bosses.TombOfSargeras.HOUNDS_OF_SARGERAS]: [
    'https://static.icy-veins.com/forum-files/uploads/monthly_2017_07/WoWScrnShot_072917_124015.jpg.8740813002cc2b68890d787157782415.jpg', // with boss + name
  ],
  [bosses.TombOfSargeras.WAR_COUNCIL]: [
    'https://static.icy-veins.com/forum-files/uploads/monthly_2017_07/WoWScrnShot_072917_000205.jpg.b9ce4e1e2d8d1e32eb8a0ec20ced7b6f.jpg', // with boss + name
  ],
  [bosses.TombOfSargeras.GAROTHI_WORLDBREAKER]: [
    'https://static.icy-veins.com/forum-files/uploads/monthly_2017_07/WoWScrnShot_072817_223530.jpg.0ccec2c807eedecde7b4470ca010a41a.jpg', // with boss + name
  ],
};
