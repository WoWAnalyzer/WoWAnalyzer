    ...
    try {
      ...
      combatants.forEach((player) => {
        if (process.env.NODE_ENV === 'development' && FAKE_PLAYER_IF_DEV_ENV) {
          console.error(
            'This player (sourceID: ' +
              player.sourceID +
              ") has an error. Because you're in development environment, we have faked the missing information, see CombatantInfoFaker.ts for more information.",
          );
    ...
    if (!player || hasDuplicatePlayers || !combatant || !combatant.specID || combatant.error) {
      ...
      return (
        <div className="container offset">
          ...
          <PlayerSelection
            players={report.friendlies
              .map((friendly) => {
                const combatant = combatants.find(
                  (combatant) => combatant.sourceID === friendly.id,
                );
                if (!combatant) {
                  return null;
                }
