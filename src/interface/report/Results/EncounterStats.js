    ...
    return fetchWcl(`rankings/encounter/${this.props.currentBoss}`, {
      ...
      .then((stats) => {
        ...
        stats.rankings.forEach((rank) => {
          ...
          rank.legendaryEffects &&
            rank.legendaryEffects.forEach((legendaryEffect) => {
              legendaries = this.addSpell(legendaries, legendaryEffect);
            });

          rank.conduitPowers &&
            rank.conduitPowers.forEach((conduit) => {
          ...
          if (rank.soulbindID) {
            if (covenants[rank.covenantID].soulbinds[rank.soulbindID]) {
              ...
              covenants[rank.covenantID].soulbinds[rank.soulbindID] = {
                id: rank.soulbindID,
                amount: 1,
              };
            }
