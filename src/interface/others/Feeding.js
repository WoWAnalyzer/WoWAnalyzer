            ...
            >
              <thead>
                <tr>
                  ...
                  <th className="text-center" colSpan="3">
                    <TooltipElement
                      content={`The amount of healing done by spells that feed into ${category.name} while it was up.`}
                    >
                      Feeding done per spell
                    </TooltipElement>
                  </th>
                  <th className="text-center">
                    <TooltipElement
                      content={`The approximated effective healing each of the spells feeding into ${category.name} did, accounting for overhealing. This should roughly at up to the total effective healing of ${category.name}.`}
                    >
                      Approx. effective healing
                    </TooltipElement>
                  </th>
                  <th />
                </tr>
              ...
              <tbody>
                {Object.keys(category.feed)
                  ...
                  .filter(
                    (spellId) => !this.state.expand || category.feed[spellId].mergedHealing > 0,
                  )
                  .map((spellId) => {
                    const ability = category.feed[spellId];
                    const healing = this.state.expand ? ability.mergedHealing : ability.healing;
                    const effectiveHealing = this.state.expand
                      ? ability.mergedEffectiveHealing
                      : ability.effectiveHealing;
                    const totalHealing = this.state.expand
                      ? category.totals.mergedTotal
                      ...
                      <tr key={`${category.name} ${ability.name}`}>
                        ...
                        <td style={{ width: '30%' }}>
                          <div
                            ...
                            style={{
                              width: `${Math.min((healing / category.max) * 100, 100)}%`,
                              backgroundColor: '#70b570',
                            }}
                          />
                        </td>
                        <td style={{ width: '10%' }}>{formatNumber(healing)}</td>
                ...
                <tr key={`${category.name}Summary`}>
                  ...
                  <td className="text-left">
                    <b>
                      {formatNumber(
                        this.state.expand ? category.totals.mergedTotal : category.totals.total,
                      )}
                    </b>
                  </td>
                  <td className="text-center">
                    <b>
                      {formatNumber(
                        this.state.expand
                          ? category.totals.mergedTotalEffective
                          : category.totals.totalEffective,
                      )}
                    </b>
                  </td>
                </tr>
