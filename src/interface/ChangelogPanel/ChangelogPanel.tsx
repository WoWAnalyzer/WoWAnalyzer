...
class ChangelogPanel extends React.PureComponent<{}, State> {
  ...
  render() {
    const limit = this.state.expanded ? undefined : 10;
      ...
      <div className="panel">
        ...
        <div className="panel-body pad">
          <select
            ...
            onChange={e =>
              this.setState({ changelogType: Number(e.target.value) })
            }
          >
          ...
          <div style={{ margin: '30px -30px 0 -30px' }}>
            <Changelog
              changelog={
                this.state.changelogType
                  ? AVAILABLE_CONFIGS.find(
                      config => config.spec.id === this.state.changelogType,
                    )!.changelog
                  : CORE_CHANGELOG
              }
              limit={limit}
