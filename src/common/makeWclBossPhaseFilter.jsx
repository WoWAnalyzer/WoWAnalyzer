...
export function makeWclBossPhaseFilter(fight) {
  ...
  if (
    ...
    const filters = [];
    const phasesForDifficulty = Object.values(bossConfig.fight.phases).filter((phase) =>
      phase.difficulties.includes(fight.difficulty),
    );
    ...
    phasesForDifficulty.forEach((phase) => {
      if (phase.filter) {
        if (phase.filter.ability) {
          filters.push(
            `(${abilityFilter[phase.filter.type] || 'ability'}.id = ${
              phase.filter.ability.id
            } AND type = "${phase.filter.type}")`,
          );
        } else if (phase.filter.health) {
          //query can't contain floats as parameters, so have to perform calculations in query itself before comparison to achieve our wanted accuracy
          const decimalFilter = createDecimalFilter(phase.filter.health);
          //find first event that marks the boss as below the threshold
          filters.push(
            `(MATCHED resources.actor.id = ${phase.filter.guid} AND (${decimalFilter.accuracy} * resources.hppercent) <= ${decimalFilter.threshold} IN (1) END)`,
          );
