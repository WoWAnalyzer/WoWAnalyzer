...
const spells = {
  ...
  // WCL logs the Runic Power generated by absorbing damage under this
  // SEPARATE spell id, not the main Anti-Magic Shell cast/buff id (48707)
  // above - same split retail's SPELLS list uses (see
  // common/SPELLS/deathknight.ts's ANTI_MAGIC_SHELL_RP_GAINED). Checking
  // 48707 for RP-gain events means it never matches, so "RP gained from AMS"
  // always reads 0.
  ...
  // The cast (43265) is a separate spell ID from the periodic ground-effect
  // damage it deals - confirmed on a live report (source correctly Lunkydk,
  // ability 52212) via classic.warcraftlogs.com's damage breakdown. The cast
  // targets "Environment" since it's ground-targeted, not a unit, but the
  // actual damage ticks are sourced by the player and hit real enemies just
  // fine under this separate ID.
  DEATH_AND_DECAY_DAMAGE: {
    id: 52212,
    name: 'Death and Decay',
    icon: 'spell_shadow_deathanddecay.jpg',
  },
  ...
  // Major glyph: Horn of Winter generates +10 RP but its cooldown is
  // increased by 100% (20s -> 40s). glyphId (1064) confirmed from the
  // auto-generated blood/spell-list_DeathKnight_Blood.classic.ts entry of
  // the same name - that's the internal id Combatant.hasGlyph() checks
  // against, NOT the spell id (146646) below.
