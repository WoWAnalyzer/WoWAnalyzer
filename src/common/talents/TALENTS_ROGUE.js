// Generated file, changes will be overwritten!
// Sun Jun 25 2017 19:03:39 GMT+0200 (W. Europe Daylight Time)

export default {
  // Shared
  ELABORATE_PLANNING_TALENT: { id: 193640, name: "Elaborate Planning", icon: "inv_misc_map08" },
  NIGHTSTALKER_TALENT: { id: 14062, name: "Nightstalker", icon: "ability_stealth" },
  SUBTERFUGE_TALENT: { id: 108208, name: "Subterfuge", icon: "rogue_subterfuge" },
  SHADOW_FOCUS_TALENT: { id: 108209, name: "Shadow Focus", icon: "rogue_shadowfocus" },
  DEEPER_STRATAGEM_TALENT: { id: 193531, name: "Deeper Stratagem", icon: "archaeology_5_0_changkiboard" },
  ANTICIPATION_TALENT: { id: 114015, name: "Anticipation", icon: "ability_rogue_slaughterfromtheshadows" },
  VIGOR_TALENT: { id: 14983, name: "Vigor", icon: "ability_rogue_vigor" },
  ELUSIVENESS_TALENT: { id: 79008, name: "Elusiveness", icon: "ability_rogue_turnthetables" },
  CHEAT_DEATH_TALENT: { id: 31230, name: "Cheat Death", icon: "ability_rogue_cheatdeath" },
  PREY_ON_THE_WEAK_TALENT: { id: 131511, name: "Prey on the Weak", icon: "ability_rogue_preyontheweak" },
  ALACRITY_TALENT: { id: 193539, name: "Alacrity", icon: "ability_paladin_speedoflight" },
  EXSANGUINATE_TALENT: { id: 200806, name: "Exsanguinate", icon: "ability_deathwing_bloodcorruption_earth" },
  MARKED_FOR_DEATH_TALENT: { id: 137619, name: "Marked for Death", icon: "achievement_bg_killingblow_berserker" },
  DEATH_FROM_ABOVE_TALENT: { id: 152150, name: "Death from Above", icon: "spell_rogue_deathfromabove" },
  // Outlaw
  GHOSTLY_STRIKE_TALENT: { id: 196937, name: "Ghostly Strike", icon: "ability_creature_cursed_02" },
  SWORDMASTER_TALENT: { id: 200733, name: "Swordmaster", icon: "inv_sword_97" },
  QUICK_DRAW_TALENT: { id: 196938, name: "Quick Draw", icon: "inv_weapon_rifle_40" },
  GRAPPLING_HOOK_TALENT: { id: 195457, name: "Grappling Hook", icon: "ability_rogue_grapplinghook" },
  ACROBATIC_STRIKES_TALENT: { id: 196924, name: "Acrobatic Strikes", icon: "spell_warrior_wildstrike" },
  HIT_AND_RUN_TALENT: { id: 196922, name: "Hit and Run", icon: "ability_rogue_fleetfooted" },
  IRON_STOMACH_TALENT: { id: 193546, name: "Iron Stomach", icon: "inv_misc_organ_11" },
  PARLEY_TALENT: { id: 199743, name: "Parley", icon: "achievement_character_human_male" },
  DIRTY_TRICKS_TALENT: { id: 108216, name: "Dirty Tricks", icon: "ability_rogue_dirtydeeds" },
  CANNONBALL_BARRAGE_TALENT: { id: 185767, name: "Cannonball Barrage", icon: "ability_rogue_cannonballbarrage" },
  KILLING_SPREE_TALENT: { id: 51690, name: "Killing Spree", icon: "ability_rogue_murderspree" },
  SLICE_AND_DICE_TALENT: { id: 5171, name: "Slice and Dice", icon: "ability_rogue_slicedice" },
  // Subtlety
  MASTER_OF_SUBTLETY_TALENT: { id: 31223, name: "Master of Subtlety", icon: "ability_rogue_masterofsubtlety" },
  WEAPONMASTER_TALENT: { id: 193537, name: "Weaponmaster", icon: "ability_ironmaidens_bladerush" },
  GLOOMBLADE_TALENT: { id: 200758, name: "Gloomblade", icon: "ability_ironmaidens_convulsiveshadows" },
  SOOTHING_DARKNESS_TALENT: { id: 200759, name: "Soothing Darkness", icon: "spell_shadow_twilight" },
  STRIKE_FROM_THE_SHADOWS_TALENT: { id: 196951, name: "Strike from the Shadows", icon: "ability_rogue_unfairadvantage" },
  TANGLED_SHADOW_TALENT: { id: 200778, name: "Tangled Shadow", icon: "inv_misc_volatileshadow" },
  DARK_SHADOW_TALENT: { id: 245687, name: "Dark Shadow", icon: "spell_warlock_demonsoul" },
  ENVELOPING_SHADOWS_TALENT: { id: 238104, name: "Enveloping Shadows", icon: "ability_rogue_envelopingshadows" },
  MASTER_OF_SHADOWS_TALENT: { id: 196976, name: "Master of Shadows", icon: "spell_shadow_charm" },
  // Assassination
  MASTER_POISONER_TALENT: { id: 196864, name: "Master Poisoner", icon: "ability_creature_poison_06" },
  HEMORRHAGE_TALENT: { id: 16511, name: "Hemorrhage", icon: "spell_shadow_lifedrain" },
  LEECHING_POISON_TALENT: { id: 108211, name: "Leeching Poison", icon: "rogue_leeching_poison" },
  THUGGEE_TALENT: { id: 196861, name: "Thuggee", icon: "inv_misc_bandana_03" },
  INTERNAL_BLEEDING_TALENT: { id: 154904, name: "Internal Bleeding", icon: "ability_rogue_bloodsplatter" },
  TOXIC_BLADE_TALENT: { id: 245388, name: "Toxic Blade", icon: "inv_weapon_shortblade_62" },
  VENOM_RUSH_TALENT: { id: 152152, name: "Venom Rush", icon: "rogue_venomzest" },
};
