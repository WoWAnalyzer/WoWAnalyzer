...
if (!WCL_API_KEY && import.meta.env.MODE === 'development') {
  console.warn(
    'Invalid WCL API key. While currently unused, in the future you may need to enter your own API key by creating a new file in the root repo called `.env.local` with the contents: `VITE_WCL_API_KEY=INSERT_YOUR_API_KEY_HERE`. After saving this file, you need to restart `pnpm run start`.',
  );
}
