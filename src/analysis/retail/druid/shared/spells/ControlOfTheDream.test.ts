...
describe('Control of the Dream', () => {
  let parser: TestCombatLogParser;
  let spellUsable: SpellUsable;
  let eventEmitter: EventEmitter;
  let statTracker: StatTracker;
  let haste: Haste;
  let abilities: Abilities;
  let controlOfTheDream: ControlOfTheDream;

  describe('Analyzer activation', () => {
    it('should be inactive without Control of the Dream talent', () => {
      ...
      // Assert
      expect(controlOfTheDream.active).toBe(false);
      // Without Control of the Dream, should get full 60s cooldown (no reduction)
      expect(spellUsable.isOnCooldown(TALENTS_DRUID.FORCE_OF_NATURE_TALENT.id)).toBe(true);
      expect(spellUsable.cooldownRemaining(TALENTS_DRUID.FORCE_OF_NATURE_TALENT.id)).toBe(
        FORCE_OF_NATURE_CD,
      );
    });

    it('should be active with Control of the Dream talent', () => {
      // Arrange
      // Without Control of the Dream
      ...
      // Assert
      expect(controlOfTheDream.active).toBe(true);
    });
  });

  describe('CD reduction', () => {
    describe('First cast (pre-pull scenario)', () => {
      ...
      it('should apply 15s reduction to first Celestial Alignment cast', () => {
        ...
        // Assert
        expect(spellUsable.isOnCooldown(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(true);
      ...
      it('should apply 15s reduction to first Convoke the Spirits cast', () => {
        ...
        // Assert
        expect(spellUsable.isOnCooldown(TALENTS_DRUID.CONVOKE_THE_SPIRITS_TALENT.id)).toBe(true);
    ...
    describe('Multi-charge abilities (Whirling Stars talent)', () => {
      ...
      it('should reduce cooldown when using first charge (was at max charges before)', () => {
        ...
        // Assert
        expect(spellUsable.chargesAvailable(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(1);
        expect(spellUsable.chargesOnCooldown(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(1);
        expect(spellUsable.cooldownRemaining(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(
      ...
      it('should NOT reduce cooldown when was NOT at max charges before', () => {
        ...
        // Assert
        expect(spellUsable.chargesAvailable(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(0);
        expect(spellUsable.chargesOnCooldown(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(2);
        expect(spellUsable.cooldownRemaining(SPELLS.CELESTIAL_ALIGNMENT.id)).toBe(
    ...
    describe('Every Balance primary CD should be tracked', () => {
      ...
      it('should track Celestial Alignment with Orbital Strike', () => {
        ...
        // Assert
        expect(spellUsable.cooldownRemaining(SPELLS.CELESTIAL_ALIGNMENT_ORBITAL_STRIKE.id)).toBe(
      ...
      it('should track Incarnation', () => {
        ...
        // Assert
        expect(spellUsable.cooldownRemaining(SPELLS.INCARNATION_CHOSEN_OF_ELUNE.id)).toBe(
      ...
      it('should track Incarnation with Orbital Strike', () => {
        ...
        // Assert
        expect(spellUsable.cooldownRemaining(SPELLS.INCARNATION_ORBITAL_STRIKE.id)).toBe(
  ...
  function InitializeWithTalents(talents: Talent[]) {
    ...
    spellUsable = parser.loadModule(SpellUsable, {
      eventEmitter: eventEmitter,
      abilities: abilities,
      priority: 1,
    }) as SpellUsable;

    // Load the ControlOfTheDream Analyzer
    controlOfTheDream = parser.loadModule(ControlOfTheDream, {
      abilities: abilities,
      spellUsable: spellUsable,
      priority: 2,
    }) as ControlOfTheDream;
  }
