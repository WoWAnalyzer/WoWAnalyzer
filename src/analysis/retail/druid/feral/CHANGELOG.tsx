...
export default [
  ...
  change(date(2021, 12, 3), <>Fixed a bug where Balance Affinity was giving Moonkin form a cooldown.</>, Sref),
  ...
  change(date(2021, 7, 9), <>Fixed an issue where only <SpellLink id={TALENTS_DRUID.INCARNATION_AVATAR_OF_ASHAMANE_FERAL_TALENT.id} /> (and not <SpellLink id={SPELLS.BERSERK.id} />) was counted as giving a stealth bonus to <SpellLink id={SPELLS.RAKE.id} /></>, Sref),
  change(date(2021, 6, 26), <>Added <SpellLink id={SPELLS.RAVENOUS_FRENZY.id}/> to CDs checklist, and fixed a bug where <SpellLink id={SPELLS.ADAPTIVE_SWARM.id}/> stats would only appear if player is specced for <SpellLink id={TALENTS_DRUID.LUNAR_INSPIRATION_FERAL_TALENT.id}/>.</>, Sref),
  change(date(2021, 6, 25), <>Added suggestions for <SpellLink id={SPELLS.TIGERS_FURY.id} /> and <SpellLink id={TALENTS_DRUID.BLOODTALONS_FERAL_TALENT.id} /> snapshotting, and updated <SpellLink id={SPELLS.FEROCIOUS_BITE.id} /> energy suggestion to be more clear about exceptions.</>, Sref),
  ...
  change(date(2021, 5, 22), <>Fixed a bug where <SpellLink id={TALENTS_DRUID.HEART_OF_THE_WILD_TALENT.id} /> cast efficiency wasn't showing properly.</>, Sref),
  ...
  change(date(2021, 5, 17), <>Updated the Checklist to include <SpellLink id={SPELLS.CONVOKE_SPIRITS.id} /> and <SpellLink id={TALENTS_DRUID.HEART_OF_THE_WILD_TALENT.id} /> cast efficiency
