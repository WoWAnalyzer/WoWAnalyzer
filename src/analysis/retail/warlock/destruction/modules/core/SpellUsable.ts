...
class SpellUsable extends CoreSpellUsable {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.applydebuff.by(SELECTED_PLAYER).spell(TALENTS.INFERNO_TALENT),
      this._handleShadowburn,
    );
    this.addEventListener(
      Events.refreshdebuff.by(SELECTED_PLAYER).spell(TALENTS.INFERNO_TALENT),
      this._handleShadowburn,
    );
  ...
  onSpendResource(event: SpendResourceEvent) {
    ...
    if (this.isOnCooldown(TALENTS.INFERNO_TALENT.id)) {
      // event.resourceChange is in multiples of 10 (2 shards = 20)
      const shardsSpent = event.resourceChange / 10;
      this.reduceCooldown(TALENTS.INFERNO_TALENT.id, shardsSpent * REDUCTION_MS_PER_SHARD);
    }
  }
  ...
  onRemoveDebuff(event: RemoveDebuffEvent) {
    ...
    if (event.ability.guid !== TALENTS.INFERNO_TALENT.id) {
      return;
    }
    const target = encodeTargetString(event.targetID, event.targetInstance);
    ...
    if (diedEarlier && this.isOnCooldown(TALENTS.INFERNO_TALENT.id)) {
      debug && this.log(`Shadowburned enemy died (${target}), cooldown reset.`);
      this.endCooldown(TALENTS.INFERNO_TALENT.id);
    }
  }
}
