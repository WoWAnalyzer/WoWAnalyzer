import SPELLS from 'common/SPELLS';
import { TALENTS_PRIEST } from 'common/TALENTS';

export const DISCIPLINE_ABILITIES_AFFECTED_BY_HEALING_INCREASES = [
  SPELLS.ATONEMENT_HEAL_NON_CRIT.id,
  SPELLS.ATONEMENT_HEAL_CRIT.id,
  SPELLS.POWER_WORD_SHIELD.id,
  SPELLS.VOID_SHIELD.id,
  TALENTS_PRIEST.EVANGELISM_TALENT.id,
  TALENTS_PRIEST.POWER_WORD_RADIANCE_TALENT.id,
  TALENTS_PRIEST.HOLY_NOVA_TALENT.id,
  SPELLS.PENANCE_BOLT_HEAL.id,
  SPELLS.PENANCE_TWINSIGHT_BOLT_HEAL.id,
  SPELLS.FLASH_HEAL.id,
  SPELLS.PLEA.id,
  // While the following spells don't double-dip in healing increases, they gain the same percentual bonus from the transfer
  SPELLS.LEECH.id,
];

export const ATONEMENT_DAMAGE_SOURCES = {
  [SPELLS.MAGIC_MELEE.id]: true, // Shadow Fiend Melee
  [SPELLS.LIGHTSPAWN_MELEE.id]: true, // Lightspawn Melee
  [SPELLS.SMITE.id]: true,
  [SPELLS.PENANCE_BOLT_DAMAGE.id]: true,
  [SPELLS.PENANCE_TWINSIGHT_BOLT_DAMAGE.id]: true,
  [SPELLS.SHADOW_WORD_PAIN.id]: true,
  [TALENTS_PRIEST.SHADOWFIEND_TALENT.id]: true,
  [TALENTS_PRIEST.HOLY_NOVA_TALENT.id]: true,
  [SPELLS.MIND_SEAR.id]: true,
  [TALENTS_PRIEST.MIND_BLAST_TALENT.id]: true,
  [SPELLS.EXPIATION_DAMAGE.id]: true,
  [TALENTS_PRIEST.SHADOW_WORD_DEATH_TALENT.id]: true,
  [SPELLS.INESCAPABLE_TORMENT_TALENT_DAMAGE.id]: true,
};

export const ATONEMENT_DAMAGE_IDS = [
  SPELLS.MAGIC_MELEE.id,
  SPELLS.LIGHTSPAWN_MELEE.id,
  SPELLS.SMITE.id,
  SPELLS.PENANCE_BOLT_DAMAGE.id,
  SPELLS.PENANCE_TWINSIGHT_BOLT_DAMAGE.id,
  SPELLS.SHADOW_WORD_PAIN.id,
  TALENTS_PRIEST.SHADOWFIEND_TALENT.id,
  TALENTS_PRIEST.HOLY_NOVA_TALENT.id,
  SPELLS.MIND_SEAR.id,
  TALENTS_PRIEST.MIND_BLAST_TALENT.id,
  SPELLS.EXPIATION_DAMAGE.id,
  TALENTS_PRIEST.SHADOW_WORD_DEATH_TALENT.id,
  TALENTS_PRIEST.INESCAPABLE_TORMENT_TALENT.id,
  SPELLS.INESCAPABLE_TORMENT_TALENT_DAMAGE.id,
  SPELLS.ULTIMATE_PENITENCE_DAMAGE.id,
  SPELLS.ENTROPIC_RIFT_DAMAGE.id,
  SPELLS.COLLAPSING_VOID_DAMAGE.id,
  SPELLS.VOID_BLAST_CAST_DISC.id,
  SPELLS.VOID_FLAY_DAMAGE_DISC.id,
];

export const EFFECTS_INCREASED_BY_BENEVOLENCE_DISCIPLINE = [
  SPELLS.ATONEMENT_HEAL_NON_CRIT.id,
  SPELLS.ATONEMENT_HEAL_CRIT.id,
  TALENTS_PRIEST.POWER_WORD_RADIANCE_TALENT.id,
  SPELLS.SHADOW_HALO_HEAL.id,
  TALENTS_PRIEST.HOLY_NOVA_TALENT.id,
  SPELLS.PENANCE_BOLT_HEAL.id,
  SPELLS.PENANCE_TWINSIGHT_BOLT_HEAL.id,
  SPELLS.CONTRITION_HEAL.id,
  SPELLS.CONTRITION_HEAL_CRIT.id,
  SPELLS.PENANCE_TWINSIGHT_BOLT_HEAL.id,
  // While the following spells don't double-dip in healing increases, they gain the same percentual bonus from the transfer
  SPELLS.LEECH.id,
  TALENTS_PRIEST.DIVINE_AEGIS_TALENT.id,
];

/**
 *
 */
export const POWER_WORD_RADIANCE_ATONEMENT_DUR = 9000;
export const POWER_WORD_SHIELD_ATONEMENT_DUR = 15000;
export const FLASH_HEAL_ATONEMENT_DUR = 15000;
export const PLEA_ATONEMENT_DUR = 15000;
export const PENANCE_ATONEMENT_DUR = 15000;
export const PAIN_AND_SUFFERING_INCREASE = [0.15, 0.3];
export const BLAZE_OF_LIGHT_INCREASE = [0.08, 0.15];

/**
 * Constants that influnce the amount of available penance bolts.
 */
export const PENANCE_BOLTS = 3;
export const PENANCE_CASTIGATION_ADDITIONAL_BOLTS = 1;
export const PENANCE_HARSH_DISIPLINE_ADDITIONAL_BOLTS_PER_RANK = [0, 1, 2];
export const PENANCE_TWINSIGHT_ADDITIONAL_BOLTS = 3;
export const PENANCE_CHANNEL_DURATION = 2600;
