...
class BlindFury extends Analyzer {
  ...
  constructor(options: Options) {
    super(options);
    this.active = this.selectedCombatant.hasTalent(BLIND_FURY_HAVOC_TALENT);
    if (!this.active) {
      return;
    }
    ...
    when(this.suggestionThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          ...
          <SpellLink id={BLIND_FURY_HAVOC_TALENT.id} /> talent to minimize Fury waste and maximize
          DPS.
        </>,
      )
        .icon(BLIND_FURY_HAVOC_TALENT.icon)
        .actual(
          <>
  ...
  statistic() {
    return (
      ...
      >
        <BoringSpellValueText spellId={BLIND_FURY_HAVOC_TALENT.id}>
          <>
            {this.badCast}{' '}
            <small>
