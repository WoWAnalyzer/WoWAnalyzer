import { FEL_ERUPTION_HAVOC_TALENT } from 'common/TALENTS/demonhunter';
...
class FelEruption extends Analyzer {
  ...
  constructor(options: Options) {
    super(options);
    this.active = this.selectedCombatant.hasTalent(FEL_ERUPTION_HAVOC_TALENT);
    if (!this.active) {
      return;
    }
    this.addEventListener(
      Events.cast.by(SELECTED_PLAYER).spell(FEL_ERUPTION_HAVOC_TALENT),
      this.countingCasts,
    );
    this.addEventListener(
      Events.applydebuff.by(SELECTED_PLAYER).spell(FEL_ERUPTION_HAVOC_TALENT),
      this.countingStuns,
    );
  }
  ...
  suggestions(when: When) {
    when(this.suggestionThresholds).addSuggestion((suggest, actual) =>
      suggest(
        <>
          Try to cast <SpellLink id={FEL_ERUPTION_HAVOC_TALENT.id} /> only for its stun. It's not
          worth casting for its damage since it's a DPS loss.
        </>,
      )
        .icon(FEL_ERUPTION_HAVOC_TALENT.icon)
        .actual(
          <>
            {actual} bad <SpellLink id={FEL_ERUPTION_HAVOC_TALENT.id} /> casts that didn't stun the
            target{' '}
          </>,
        )
  ...
  statistic() {
    return (
      ...
      >
        <BoringSpellValueText spellId={FEL_ERUPTION_HAVOC_TALENT.id}>
          <>
