...
import { DEMON_BLADES_HAVOC_TALENT } from 'common/TALENTS/demonhunter';
...
class DemonBlades extends Analyzer {
  ...
  constructor(options: Options) {
    super(options);
    this.active = this.selectedCombatant.hasTalent(DEMON_BLADES_HAVOC_TALENT.id);
    if (!this.active) {
      return;
    }
    ...
    when(this.suggestionThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          ...
          <SpellLink id={DEMON_BLADES_HAVOC_TALENT.id} />.
        </>,
      )
        .icon(DEMON_BLADES_HAVOC_TALENT.icon)
        .actual(
          t({
  ...
  statistic() {
    ...
    return (
      ...
      >
        <BoringSpellValueText spellId={DEMON_BLADES_HAVOC_TALENT.id}>
          <>
