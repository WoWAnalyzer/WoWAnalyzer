...
class Abilities extends CoreAbilities {
  spellbook(): SpellbookAbility[] {
    ...
    return [
        ...
        castEfficiency: {
          ...
          extraSuggestion: (
            <>
              ...
              using the <SpellLink id={FALLOUT_VENGEANCE_TALENT.id} /> talent in order to maximize
              your <SpellLink id={SPELLS.SOUL_FRAGMENT.id} /> generation.
            </>
          ),
        },
      },
      {
        spell: [
          combatant.hasTalent(FRACTURE_VENGEANCE_TALENT.id)
            ? FRACTURE_VENGEANCE_TALENT.id
            : SPELLS.SHEAR.id,
        ],
        category: SPELL_CATEGORY.ROTATIONAL,
        cooldown: combatant.hasTalent(FRACTURE_VENGEANCE_TALENT.id)
          ? (haste) => 4.5 / (1 + haste)
          : 0,
        charges: combatant.hasTalent(FRACTURE_VENGEANCE_TALENT.id) ? 2 : 0,
        castEfficiency: {
          suggestion: combatant.hasTalent(FRACTURE_VENGEANCE_TALENT.id),
          recommendedEfficiency: 0.9,
        },
        gcd: {
          base: 1500,
      ...
      // Talents
      {
        spell: SIGIL_OF_CHAINS_VENGEANCE_TALENT.id,
        category: SPELL_CATEGORY.UTILITY,
        enabled: combatant.hasTalent(SIGIL_OF_CHAINS_VENGEANCE_TALENT.id),
        cooldown: 90,
        gcd: {
          base: 1500,
        },
      ...
      {
        spell: SPIRIT_BOMB_VENGEANCE_TALENT.id,
      ...
      {
        spell: SOUL_BARRIER_VENGEANCE_TALENT.id,
        buffSpellId: SOUL_BARRIER_VENGEANCE_TALENT.id,
        enabled: combatant.hasTalent(SOUL_BARRIER_VENGEANCE_TALENT.id),
        category: SPELL_CATEGORY.DEFENSIVE,
        cooldown: 30,
        gcd: {
          base: 1500,
      ...
      // Sigils
      {
        ...
        category: SPELL_CATEGORY.UTILITY,
        cooldown: 60 * (1 - (combatant.hasTalent(QUICKENED_SIGILS_TALENT.id) ? 0.2 : 0)),
        gcd: {
          base: 1500,
        },
      },
      ...
      },
      {
        ...
        category: SPELL_CATEGORY.ROTATIONAL_AOE,
        cooldown: 30 * (1 - (combatant.hasTalent(QUICKENED_SIGILS_TALENT.id) ? 0.2 : 0)),
        gcd: {
          base: 1500,
        },
        enabled: !(
      ...
      // Covenant (move these if needed)
      {
        ...
        enabled: combatant.hasTalent(ELYSIAN_DECREE_VENGEANCE_TALENT),
        castEfficiency: {
          suggestion: true,
          recommendedEfficiency: 0.9,
        },
