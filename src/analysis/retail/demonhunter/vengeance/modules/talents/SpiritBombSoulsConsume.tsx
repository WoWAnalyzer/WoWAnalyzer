...
*/
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.cast.by(SELECTED_PLAYER).spell(SPIRIT_BOMB_VENGEANCE_TALENT),
      this.onCast,
    );
    this.addEventListener(
  ...
  suggestions(when: When) {
    when(this.suggestionThresholdsEfficiency).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          Try to cast <SpellLink id={SPIRIT_BOMB_VENGEANCE_TALENT.id} /> at 4 or 5 souls.
        </>,
      )
        .icon(SPIRIT_BOMB_VENGEANCE_TALENT.icon)
        .actual(
          t({
  ...
  statistic() {
    return (
      ...
      >
        <BoringSpellValueText spellId={SPIRIT_BOMB_VENGEANCE_TALENT.id}>
