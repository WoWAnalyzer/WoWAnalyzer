...
class FeedTheDemon extends Analyzer {
  ...
  onSpendResource(event: SpendResourceEvent) {
    const furySpent = event.resourceChange;
    if (!furySpent || furySpent <= 0) {
      return;
    }

    this.furyAccumulator += furySpent;
    while (this.furyAccumulator >= FURY_NEEDED_FOR_CDR) {
      this.furyAccumulator -= FURY_NEEDED_FOR_CDR;
