...
//Need this variant to change how passive rune regeneration is displayed.
const RuneBreakdown = ({ tracker, showSpenders }: Props) => {
  const resourceName = tracker.resource.name;
  const generated = prepareGenerated(tracker);
  const spent = prepareSpent(tracker);
          ...
          {generated?.map((ability) => (
            <tr key={ability.abilityId}>
              <td style={{ width: '30%' }}>
                ...
                {ability.abilityId !== SPELLS.RUNE_1.id && <SpellLink spell={ability.abilityId} />}
              </td>
