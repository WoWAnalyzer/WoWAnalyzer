...
class HammerofWrath extends ExecuteHelper {
  ...
  constructor(options: Options) {
    ...
    const baseCD =
      {
        [SPECS.PROTECTION_PALADIN.id]: 5,
        [SPECS.RETRIBUTION_PALADIN.id]: 11,
        [SPECS.HOLY_PALADIN.id]: 6,
      }[specId] ?? 11;
  ...
  adjustMaxCasts(event: FightEndEvent) {
    const cooldown = this.abilities.getAbility(SPELLS.HAMMER_OF_WRATH_CAST.id)!.cooldown * 1000;
