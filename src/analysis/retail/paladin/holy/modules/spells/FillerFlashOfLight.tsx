...
class FillerFlashOfLight extends Analyzer {
  ...
  _isInefficientCastEvent(event: BeginCastEvent) {
    ...
    // The `-1` buffer time is to properly handle chain-casting and IoL buffs; when chain casting the first FoL will consume the IoL buff on the `cast` event and that exact same frame will have the `begincast` event. Because `has2SetBuff` looks at the timestamp rather than event order, it would otherwise include the buff.
