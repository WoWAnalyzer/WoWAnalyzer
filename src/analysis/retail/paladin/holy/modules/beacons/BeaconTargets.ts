...
class BeaconTargets extends Analyzer {
  ...
  constructor(options: Options) {
    super(options);
    if (this.selectedCombatant.hasTalent(TALENTS.BEACON_OF_FAITH_HOLY_TALENT.id)) {
      this.maxBeacons = 2;
    } else if (this.selectedCombatant.hasTalent(TALENTS.BEACON_OF_VIRTUE_HOLY_TALENT.id)) {
      this.maxBeacons = 4;
    }
