...
import { getChainHeals, wasRiptideConsumed } from './EventLinkNormalizer';
...
class ChainHealAnalyzer extends Analyzer {
  ...
  private calculateBaseChainHeal(event: HealEvent, cast: CastEvent): BufferHealEvent {
    ...
    const choiceTalent = this.selectedCombatant.hasTalent(TALENTS.FLOW_OF_THE_TIDES_TALENT)
      ? TALENTS.FLOW_OF_THE_TIDES_TALENT
      : this.selectedCombatant.hasTalent(TALENTS.ANCESTRAL_REACH_TALENT)
        ? TALENTS.ANCESTRAL_REACH_TALENT
        : null;
    ...
    switch (choiceTalent) {
      case TALENTS.FLOW_OF_THE_TIDES_TALENT: {
      ...
      case TALENTS.ANCESTRAL_REACH_TALENT: {
        //check for ANCESTRAL REACH increase
