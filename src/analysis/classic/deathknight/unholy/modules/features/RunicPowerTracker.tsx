...
/**
 * Unholy Presence RP mechanics:
 *   Base rune RP generation: 10 RP per rune spent.
 *   Death Coil costs 32 RP (reduced by Unholy Presence vs baseline).
 *   ERW generates 25 RP (standard — no Frost Presence bonus).
 *
 * We simulate RP completely ourselves rather than reading WCL's classResources.
 */

/** Total runes consumed per ability — drives base RP generation. */
const RUNE_COUNTS: Partial<Record<number, number>> = {
  ...
  [SPELLS.SOUL_REAPER_UNHOLY.id]: 1,
...
/** RP cost for abilities that spend Runic Power (Unholy Presence costs). */
const RP_COSTS: Partial<Record<number, number>> = {
  [SPELLS.DEATH_COIL_DK.id]: 32, // Unholy Presence reduces from baseline
  [SPELLS.FROST_STRIKE.id]: 40, // Rarely used by Unholy, baseline cost
};

/**
 * Tracks simulated Runic Power for an Unholy DK.
 * Base rune RP gains are calculated from ability rune costs (10 RP/rune).
 * AMS RP gain is read from WCL resourcechange events (cannot be simulated).
 */
class RunicPowerTracker extends Analyzer {
  /** RP generated per rune spent (base, no presence bonus) */
  static readonly RP_PER_RUNE = 10;
  /** ERW RP gain — standard (no Frost Presence) */
  static readonly ERW_RP_GAIN = 25;
  ...
  private onCast(event: CastEvent): void {
    ...
    // ERW: standard 25 RP for Unholy DK
    ...
    // Rune-spending abilities: 10 RP per rune
