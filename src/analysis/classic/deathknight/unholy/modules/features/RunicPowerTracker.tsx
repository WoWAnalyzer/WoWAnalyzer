...
/**
 * Tracks Unholy DK Runic Power via the core ResourceTracker.
 *
...
class RunicPowerTracker extends ResourceTracker {
  ...
  /** Being an Unholy Death Knight passively reduces Death Coil's RP cost by 8 (real units; 80 at our 10x scale). */
  private static readonly DEATH_COIL_UNHOLY_DISCOUNT = 80;

  /**
   * WCL reports Death Coil's *base* RP cost (40 RP / 400 at our scale) - it doesn't
   * account for the passive discount baked into the Unholy spec itself (not tied to
   * a buff/presence, so we just apply it unconditionally for this ability).
   *
  ...
  getAdjustedCost(event: CastEvent): number | undefined {
    ...
    if (cost !== undefined && event.ability.guid === SPELLS.DEATH_COIL_DK.id) {
      return Math.max(0, cost - RunicPowerTracker.DEATH_COIL_UNHOLY_DISCOUNT);
    }
    return cost;
  }
  ...
  /** Frost Presence increases RP generation by 20% (applies regardless of spec if the buff is up). */
