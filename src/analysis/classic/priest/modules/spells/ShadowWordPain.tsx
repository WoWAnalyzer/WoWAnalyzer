...
class ShadowWordPain extends Analyzer {
  ...
  shadowWeavingStacksPercentByRank(rank: number) {
    ...
    return (
      this.shadowWeavingDurationByStack[rank] / this.enemies.getBuffUptime(SPELLS.SHADOW_WORD_PAIN)
    );
  }
  ...
  swpApplication(event: ApplyDebuffEvent) {
    ...
    this.shadowWordPainTracker[
      event.targetID
    ].shadowWeavingStackCount = this.currentShadowWeavingStacks;
  }
  ...
  suggestions(when: When) {
    ...
    when(this.shadowWeavingSuggestionThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <span>
          You should apply <SpellLink id={SPELLS.SHADOW_WORD_PAIN} /> when you have 5 stacks of{' '}
          <SpellLink id={SHADOW_WEAVING_BUFF} />. The damage of{' '}
          <SpellLink id={SPELLS.SHADOW_WORD_PAIN} /> snapshots when you apply it, but not when you
          refresh it. The more stacks of <SpellLink id={SHADOW_WEAVING_BUFF} /> you have when
          applying SWP, the better.
        </span>,
      )
        .icon('spell_shadow_shadowwordpain')
        .actual(
    ...
    when(this.shadowWordPainOverwriteThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <span>
          <SpellLink id={SPELLS.SHADOW_WORD_PAIN} /> is automatically refreshed by{' '}
          <SpellLink id={SPELLS.PAIN_AND_SIFFERING_TALENT} />. Try to avoid hardcasting{' '}
          <SpellLink id={SPELLS.SHADOW_WORD_PAIN} /> unless you cast it with less than 5 stacks of{' '}
          <SpellLink id={SHADOW_WEAVING_BUFF} />
        </span>,
      )
        .icon('spell_shadow_shadowwordpain')
        .actual(
