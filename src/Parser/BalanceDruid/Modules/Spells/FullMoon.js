...
class FullMoon extends Module {
    ...
    on_byPlayer_cast(event) {
        ...
        if (spellId !== SPELLS.FULL_MOON.id)
            this.fullMoonOrder ++;
        else
    ...
    suggestions(when) {
        ...
        if (extraMoons > this.fullMoonOrder) fmAvailableCasts ++;
        ...
        .addSuggestion((suggest, actual, recommended) => {
            return suggest(<span> You casted {Math.round(formatPercentage(actual))}% of total available <SpellLink id={SPELLS.FULL_MOON.id} />, unless you have extended periods of downtime you should cast {Math.round(formatPercentage(recommended))}%.</span>)
