...
class ResourceBreakdown extends React.Component {
  ...
  prepareGenerated(pointGen, pointWasted) {
    return Object.keys(pointGen)
      .map(abilityId => ({
        ...
        generated: pointGen[abilityId].total,
        wasted: pointWasted[abilityId].total,
      }))
  ...
  prepareSpent(pointSpent, pointCast) {
    return Object.keys(pointSpent)
      .map(abilityId => ({
        abilityId: Number(abilityId),
        spent: pointSpent[abilityId].total,
  ...
  render() {
    const { pointsGained, pointsSpent, pointsWasted, pointsCast, resourceName } = this.props;
