...
class Tier21_4set extends Analyzer {
  ...
  on_initialized() {
      this.active = this.owner.modules.combatants.selected.hasBuff(SPELLS.HOLY_PRIEST_T21_4SET_BONUS_BUFF.id);
  }

  on_byPlayer_heal(event) {
      const spellId = event.ability.guid;
      const hasBuff = this.owner.modules.combatants.selected.hasBuff(SPELLS.HOLY_PRIEST_T21_4SET_BUFF.id, event.timestamp, HOLY_PRIEST_TIER21_4SET_BUFF_EXPIRATION_BUFFER);
