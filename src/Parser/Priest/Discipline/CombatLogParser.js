...
import AtonementSuccessiveDamageNormalizer2 from './Normalizers/AtonementSuccessiveDamage';
...
class CombatLogParser extends CoreCombatLogParser {
  ...
  static specModules = {
    ...
    atonementSuccessiveDamage2: AtonementSuccessiveDamageNormalizer2,
  ...
  generateResults() {
    ...
    if (deadTimePercentage > 0.2) {
      results.addIssue({
        issue: `Your downtime can be improved. Try to Always Be Casting (ABC); when there's nothing to heal try to contribute some damage (${Math.round(deadTimePercentage * 100)}% downtime).`,
