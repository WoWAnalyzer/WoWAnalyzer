...
class FuryOfNature extends Analyzer {
  ...
  constructor(...args) {
    super(...args);
    this.active = this.selectedCombatant.hasBack(ITEMS.FURY_OF_NATURE.id);
  }

  on_byPlayer_damage(event) {
    if (this.selectedCombatant.hasBuff(SPELLS.BEAR_FORM.id) && (event.ability.type === MAGIC_SCHOOLS.ids.NATURE || event.ability.type === MAGIC_SCHOOLS.ids.ARCANE)) {
