...
class ReapAndSow extends Module {
  ...
  on_byPlayer_applybuff(event) {
    if (event.ability.guid === SPELLS.WARLOCK_TORMENTED_SOULS.id) {
      this.tormentedSoulCounter = 1;
    }
  }

  on_byPlayer_applybuffstack(event) {
    if (event.ability.guid === SPELLS.WARLOCK_TORMENTED_SOULS.id) {
      this.tormentedSoulCounter = event.stack;
    }
  }
  ...
  on_byPlayer_cast(event) {
    ...
    if (this.totalTimeGained === 0) {
      this.totalTimeGained += this.tormentedSoulCounter * BONUS_BUFF_TIME_PER_SOUL;
      this.expectedBuffEndTimestamp = event.timestamp + this.tormentedSoulCounter * BUFF_TIME_PER_SOUL;
    }
    else {
      ...
      if (event.timestamp >= this.expectedBuffEndTimestamp) {
        ...
        const remainingDuration = this.expectedBuffEndTimestamp - event.timestamp;
        const buffDuration = this.tormentedSoulCounter * BUFF_TIME_PER_SOUL;
        ...
        if (remainingDuration + buffDuration > MAX_BUFF_DURATION) {
          ...
          const timeTillMax = maxBuffTimestamp - this.expectedBuffEndTimestamp;
          ...
          //                                ^    MAX
          //                                |
          const maxBuffWithoutLastBonusTimestamp = this.expectedBuffEndTimestamp + numberOfFullBuffsTillMax * BUFF_TIME_PER_SOUL + BASE_BUFF_TIME_PER_SOUL; //honestly I ran out of ideas how to call it
          ...
          this.expectedBuffEndTimestamp = maxBuffTimestamp;
        }
        else {
          effectiveBonusTimeGained = this.tormentedSoulCounter * BONUS_BUFF_TIME_PER_SOUL;
          this.expectedBuffEndTimestamp = event.timestamp + this.tormentedSoulCounter * BUFF_TIME_PER_SOUL;
        }
        this.totalTimeGained += effectiveBonusTimeGained;
      }
    }
    this.tormentedSoulCounter = 0;
  }
