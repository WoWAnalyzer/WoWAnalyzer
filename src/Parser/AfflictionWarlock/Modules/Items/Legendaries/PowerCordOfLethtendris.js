...
class PowerCordOfLethtendris extends Module {
  ...
  item() {
    ...
    return {
      ...
      result: (
        <dfn data-tip={`This result is estimated by multiplying number of Soul Shards gained from this item (${shardsGained} Soul Shards gained) by the average Unstable Affliction damage for the whole fight.`}>
          {`${formatNumber(estimatedUAdamage)} damage - ${this.owner.formatItemDamageDone(estimatedUAdamage)}`}
        </dfn>
      ),
    };
