...
class Tier20_2p extends Module {
  ...
  on_byPlayer_damage(event) {
    if(this.combatants.selected.hasBuff(SPELLS.HUNTER_MM_T20_2P_BONUS_BUFF.id) && (event.ability.guid === SPELLS.AIMED_SHOT.id)) {
      this.buffedAimed += 1;
      this.totalAimed += 1;
    }
    if(!this.combatants.selected.hasBuff(SPELLS.HUNTER_MM_T20_2P_BONUS_BUFF.id) && (event.ability.guid === SPELLS.AIMED_SHOT.id)) {
  ...
  item() {
    return {
      ...
      result: (
<dfn data-tip={`Your utilization of tier 20 2 piece: <br/>
Buffed aimed shots: ${this.buffedAimed}.<br/>
Total aimed shots:  ${this.totalAimed}.<br/> `}>
