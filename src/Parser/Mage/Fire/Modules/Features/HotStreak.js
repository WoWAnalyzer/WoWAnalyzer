...
class HotStreak extends Analyzer {
  ...
  on_byPlayer_cast(event) {
    ...
    if (spellId === SPELLS.PHOENIXS_FLAMES.id) {this.phoenixFlamesHits = 0;}
    if (spellId === SPELLS.DRAGONS_BREATH.id) {this.dragonsBreathHits = 0;}
  }
  ...
  on_byPlayer_removebuff(event) {
    ...
    if(!this.lastCastTimestamp || this.lastCastTimestamp + PROC_WINDOW_MS < this.owner.currentTimestamp) {
      this.expiredProcs += 1;
      if(debug) { console.log("Hot Streak proc expired @ " + formatMilliseconds(event.timestamp - this.owner.fight.start_time)); }
    }
  }
