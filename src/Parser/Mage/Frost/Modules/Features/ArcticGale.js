...
class ArcticGale extends Analyzer {
  ...
  suggestions(when) {
    when(this.damage).isLessThanOrEqual(0)
      .addSuggestion((suggest, actual, recommended) => {
        return suggest(<span>You took <SpellLink id={SPELLS.ARCTIC_GALE_TALENT.id}/> but never used <SpellLink id={SPELLS.BLIZZARD.id}/>. Consider taking a different talent.</span>)
