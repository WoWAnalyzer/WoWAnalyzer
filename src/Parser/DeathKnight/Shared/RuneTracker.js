...
class RuneTracker extends ResourceTracker {
  ...
  get runesMaxCasts() {
    ...
    for (const builder in this.buildersObj) { //subtract runes generated by other sources
      const asInt = parseInt(builder, 10);
      // do not include itself in the calculation
      if (asInt !== SPELLS.RUNE_1.id && asInt !== SPELLS.RUNIC_CORRUPTION.id && asInt !== SPELLS.RUNE_MASTER.id) {
        totalCasts -= this.buildersObj[builder].generated;
      }
    }
