...
class BattleCrySetupAnalyzer extends BattleCryAnalyzer {
  ...
  suggestions(when) {
    when(this.shatteredSetupThresholds).addSuggestion((suggest, actual, recommended) => {
      return suggest(<React.Fragment>Try to have <SpellLink id={SPELLS.SHATTERED_DEFENSES.id} icon /> active before you use <SpellLink id={SPELLS.RECKLESSNESS.id} icon /> to maximize your burst potential.</React.Fragment>)
