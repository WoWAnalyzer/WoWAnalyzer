...
class SlamAnalyzer extends Analyzer {
  ...
  on_byPlayer_cast(event) {
    ...
    if(this.selectedCombatant.hasTalent(SPELLS.FERVOR_OF_BATTLE_TALENT.id)) {
      // If Fervor of Battle is talented stop here because every slam is going to be flagged as bad.
      return;
    }

    const weightedBlades3Stacks = this.selectedCombatant.hasBuff(SPELLS.WEIGHTED_BLADES.id) && this.selectedCombatant.getBuff(SPELLS.WEIGHTED_BLADES.id).stacks === 3;

    if(!this.selectedCombatant.hasBuff(SPELLS.SHATTERED_DEFENSES.id) && this.spellUsable.isAvailable(SPELLS.COLOSSUS_SMASH.id)) {
