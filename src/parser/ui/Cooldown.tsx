...
const CooldownComponent = ({ fightStart, fightEnd, cooldown, applyTimeFilter }: Props) => {
  const [showCastEvents, setShowCastEvents] = useState(false);
  const [showAllEvents, setShowAllEvents] = useState(false);

  const handleExpandClick = () => {
    setShowCastEvents(!showCastEvents);
    setShowAllEvents(false);
  };

  const handleShowHealsClick = () => {
    setShowAllEvents(!showAllEvents);
  };

  const groupHeals = (events: AnyEvent[]): (CastEvent | HealData)[] => {
  ...
  const calculateHealingStatistics = (cooldown: Pick<Cooldown, 'events'>) => {
    let healingDone = 0;
  ...
  const calculateDamageStatistics = (cooldown: Pick<Cooldown, 'events'>) => {
  ...
  const formatRelativeTimestamp = (event: AnyEvent, cooldown: Cooldown) => {
  ...
  let healingStatistics: ReturnType<typeof calculateHealingStatistics> | null = null;
  let extendedHealingStatistics: ReturnType<typeof calculateHealingStatistics> | null = null;
    ...
    <>
      <article>
        ...
        <div className="row" style={{ width: '100%' }}>
          <div className={showAllEvents ? 'col-md-12' : 'col-md-6'}>
            <header style={{ marginTop: 5, fontSize: '1.25em', marginBottom: '.1em' }}>
              ...
              >
                <a href="#" onClick={() => applyTimeFilter?.(start - fightStart, end - fightStart)}>
              ...
              <div>
                ...
                <div className="row">
                  <div className="col-xs-12">
                    <a href="#" onClick={handleExpandClick} style={{ marginTop: '.2em' }}>
            ...
            {showCastEvents && !showAllEvents && (
              <div className="container-fluid">
                ...
                <div className="row">
                  <div className="col-xs-12">
                    <a href="#" onClick={handleShowHealsClick} style={{ marginTop: '.2em' }}>
            ...
            {showCastEvents && showAllEvents && (
              <div className="container-fluid">
                {groupHeals(
                  ...
                  const event: HealEvent | CastEvent = 'event' in rawHeal ? rawHeal.event : rawHeal;
                ...
                {cooldown.summary.map((item) => {
                  switch (item) {
                    case BUILT_IN_SUMMARY_TYPES.HEALING:
                      healingStatistics = healingStatistics || calculateHealingStatistics(cooldown);
                    ...
                    case BUILT_IN_SUMMARY_TYPES.MANA: {
                      ...
                      return (
                        <div className="col-md-4 text-center" key="mana-used">
                          <Trans id="shared.cooldownThroughputTracker.cooldown.manaUsed" key="mana">
                    ...
                    case BUILT_IN_SUMMARY_TYPES.DAMAGE: {
                      const damageStatistics = calculateDamageStatistics(cooldown);
                ...
                <div className="row">
                  <div className="col-md-12">
                    <header style={{ marginTop: 5, fontSize: '1.25em', marginBottom: '.1em' }}>
                      ...
                      {formatDuration(
                        cooldown.lateEvents[cooldown.lateEvents.length - 1].timestamp - fightStart,
                      )}
                      )
                    </header>
                  </div>
                    ...
                    switch (item) {
                      case BUILT_IN_SUMMARY_TYPES.HEALING:
                        extendedHealingStatistics =
                          extendedHealingStatistics ||
                          calculateHealingStatistics(fakeSummaryCooldown);
                      ...
                      case BUILT_IN_SUMMARY_TYPES.DAMAGE: {
                        const damageStatistics = calculateDamageStatistics(fakeSummaryCooldown);
                        return (
                          <div className="col-md-4 text-center" key="damage">
                            <div style={{ fontSize: '2em' }}>
                              {formatNumber(damageStatistics.damageDone)}
