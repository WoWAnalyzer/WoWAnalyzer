...
const PlayerBreakdown = ({ report, spellreport, players }: Props) => {
  ...
  const calculateSpellBreakdown = (statsBySpellId: Record<string, SpellStats>) => {
  ...
  const friendlyStats = calculatePlayerBreakdown(report, players);
  ...
  if (spellreport) {
    spellStats = calculateSpellBreakdown(spellreport);
