...
export type EventLink = {
  ...
  additionalCondition?: (linkingEvent: MappedEvent, referencedEvent: MappedEvent) => boolean;
...
abstract class EventLinkNormalizer extends EventsNormalizer {
  ...
  private _isLinking(el: EventLink, event: MappedEvent): boolean {
  ...
  private _isReferenced(el: EventLink, event: MappedEvent): boolean {
  ...
  private _sourceCheck(el: EventLink, event1: MappedEvent, event2: MappedEvent): boolean {
    return (
      el.anySource ||
  ...
  private _targetCheck(el: EventLink, event1: MappedEvent, event2: MappedEvent): boolean {
    return (
      el.anyTarget ||
      (HasTarget(event1) &&
        HasTarget(event2) &&
  ...
  private _checkAndLink(
    el: EventLink,
    linkingEvent: MappedEvent,
    referencedEvent: MappedEvent,
  ): number {
    if (
