...
export interface SpellbookBuff {
  /**
...
class Buff {
  ...
  spellId!: SpellbookBuff['spellId'];
  enabled: SpellbookBuff['enabled'] = true;
  triggeredBySpellId: SpellbookBuff['triggeredBySpellId'];
  timelineHighlight: SpellbookBuff['timelineHighlight'];
   ...
   * @param options
   */
  constructor(options: SpellbookBuff) {
    this._setProps(options);
  }

  _setProps(props: SpellbookBuff) {
    if (process.env.NODE_ENV === 'development') {
      ...
      PropTypes.checkPropTypes(
        ...
        (this.constructor as typeof Buff).propTypes,
        props,
        'prop',
        'Buff',
      );
      Object.keys(props).forEach((prop) => {
        if (
          // eslint-disable-next-line react/forbid-foreign-prop-types
          (this.constructor as typeof Buff).propTypes[prop] === undefined
        ) {
          console.log(prop);
          throw new Error(
