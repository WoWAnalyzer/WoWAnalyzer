...
export interface SpellbookDebuff {
  /**
   ...
   * ability can be used to tie multiple cast / debuff IDs together as the same ability
  ...
  enabled?: boolean;
  /**
   * The spells that trigger this debuff. Defaults to the same spell as the debuff (this is most
...
class Debuff {
  static propTypes: { [key: string]: any } = {
    /**
     * REQUIRED The spell id. If an array of spell ids is provided, the first element in the array will be what shows in suggestions / cast timeline. Multiple spell definitions in the same ability can be used to tie multiple cast / debuff IDs together as the same ability (with a shared cooldown)
     */
    ...
    /**
     * The spells that trigger this debuff. Defaults to the same spell as the debuff (this is most commonly the same spell). Only configure this if it's different.
     */
  ...
  spellId!: SpellbookDebuff['spellId'];
  enabled: SpellbookDebuff['enabled'] = true;
  triggeredBySpellId: SpellbookDebuff['triggeredBySpellId'];
  timelineHighlight: SpellbookDebuff['timelineHighlight'];
   ...
   * @param options
   */
  constructor(options: SpellbookDebuff) {
    this._setProps(options);
  }

  _setProps(props: SpellbookDebuff) {
    if (process.env.NODE_ENV === 'development') {
      ...
      PropTypes.checkPropTypes(
        ...
        (this.constructor as typeof Debuff).propTypes,
        props,
        'prop',
        'Debuff',
      );
      Object.keys(props).forEach((prop) => {
        if (
          // eslint-disable-next-line react/forbid-foreign-prop-types
          (this.constructor as typeof Debuff).propTypes[prop] === undefined
        ) {
          console.log(prop);
          throw new Error(
