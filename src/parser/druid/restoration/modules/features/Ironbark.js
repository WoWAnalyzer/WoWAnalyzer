...
class Ironbark extends Analyzer {
  ...
  loadDamageTakenDuringIronbark() {
    fetchWcl(`report/tables/damage-taken/${this.owner.report.code}`, {
      ...
      .then((json) => {
        this.damageTakenDuringIronbark = json.entries.reduce(
    ...
    if (this.damageTakenDuringIronbark) {
      return (
        <Statistic
          ...
          tooltip={
            <>
              This is the average amount of damage you prevented per Ironbark cast. The total damage
              prevented over your <strong>{this.ironbarkCount} casts</strong> was{' '}
              <strong>{formatNumber(this.damageReduced)}</strong>. While this amount is not counted
              in your healing done, this is equivalent to{' '}
              <strong>
                {formatPercentage(this.owner.getPercentageOfTotalHealingDone(this.damageReduced))}%
              </strong>{' '}
        ...
        >
          <BoringValue
            label={
              <>
                <SpellIcon id={SPELLS.IRONBARK.id} /> Average Ironbark mitigation
              </>
            }
          >
