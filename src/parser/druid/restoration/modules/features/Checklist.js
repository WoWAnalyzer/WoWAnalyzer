  ...
  rules = [
    ...
    new Rule({
      ...
      requirements: () => {
        return [
          new Requirement({
            name: <><SpellLink id={SPELLS.WILD_GROWTH.id} /> / <SpellLink id={SPELLS.REJUVENATION.id} /> ratio</>,
    ...
    new Rule({
      ...
      description: <>Your cooldowns can be a big contributor to healing throughput when used frequently throughout the fight. When used early and often they can contribute a lot of healing for very little mana. Try to plan your major cooldowns like (<SpellLink id={SPELLS.TRANQUILITY_CAST.id} /> around big damage boss abilities, like the Transition Phase on Imonar or Fusillade on Antoran High Command. The below percentages represent the percentage of time you kept each spell on cooldown.</>,
      requirements: () => {
    ...
    new Rule({
      ...
      description: <>Casting on targets who don't need healing or recklessly using inefficienct heals can result in running out of mana well before an encounter ends. Adapt your spell use to the situation, and as a rule of thumb try and keep your current mana percentage just above the bosses health percentage.</>,
      requirements: () => {
        return [
          new Requirement({
            name: <>Mana saved during <SpellLink id={SPELLS.INNERVATE.id} /></>,
          ...
          new Requirement({
            name: <>Use your <SpellLink id={SPELLS.CLEARCASTING_BUFF.id} /> procs</>,
          ...
          new Requirement({
            name: <>Don't overuse <SpellLink id={SPELLS.REGROWTH.id} /></>,
    ...
    new Rule({
      ...
      description: <>Restoration Druids unfortunately do not have many tools to deal with burst damage, but you should take care to use the ones you have. Swiftmend is a fairly inefficient spell, and should only be used in an emergency. The below percentages represent the percentage of time you kept each spell on cooldown.</>,
      requirements: () => {
        return [
          new GenericCastEfficiencyRequirement({
    ...
    new Rule({
      ...
      description: <>Different talent choices can be more or less effective depending on the fight. Listed below you will see how much throughput some talents were providing.</>,
      requirements: () => {
        return [
          new Requirement({
