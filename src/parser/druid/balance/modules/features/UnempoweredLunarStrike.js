...
class UnempoweredLunarStrike extends Analyzer {
  ...
  constructor(options) {
    ...
    this.addEventListener(
      Events.damage.by(SELECTED_PLAYER).spell(SPELLS.LUNAR_STRIKE),
      this.onDamage,
    );
  ...
  onCast(event) {
    ...
    this.lastCast = event;
    this.lastCastBuffed =
      this.selectedCombatant.hasBuff(SPELLS.LUNAR_EMP_BUFF.id) ||
  ...
  suggestions(when) {
    when(this.suggestionThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You cast {this.badCasts} unempowered and non instant cast{' '}
          <SpellLink id={SPELLS.LUNAR_STRIKE.id} /> that hit less than {TARGETS_FOR_GOOD_CAST}{' '}
          targets. Always prioritize <SpellLink id={SPELLS.SOLAR_WRATH.id} /> as a filler when none
