...
interface Props {
  ...
  scaleFactor?: number;
  /** Override how builder rows are computed (e.g., to merge abilities). Defaults to {@link prepareGenerated}. */
  prepareGenerated?: (tracker: ResourceTracker, scaleFactor?: number) => GeneratedResourceRow[];
  /** Override how spender rows are computed. Defaults to {@link prepareSpent}. */
  prepareSpent?: (tracker: ResourceTracker, scaleFactor?: number) => SpentResourceRow[];
}
...
export interface GeneratedResourceRow extends ResourceUsageRow {
...
export interface SpentResourceRow extends ResourceUsageRow {
...
export const prepareSpent = (tracker: ResourceTracker, scaleFactor = 1): SpentResourceRow[] =>
  Object.keys(tracker.spendersObj)
    .map((abilityId) => ({
      abilityId: Number(abilityId),
...
const ResourceBreakdown = ({
  ...
  hideGenerated,
  scaleFactor = 1,
  prepareGenerated: prepareGeneratedProp = prepareGenerated,
  prepareSpent: prepareSpentProp = prepareSpent,
}: Props) => {
  const resourceName = tracker.resource.name;

  const generated = prepareGeneratedProp(tracker, scaleFactor);
  const spent = prepareSpentProp(tracker, scaleFactor);
