...
const groupHealingBySeconds = (
  healingBySecond: Record<number, HealingValue>,
  interval: number,
): Record<number, HealingValue> =>
  Object.keys(healingBySecond)
    .map(Number)
...
const HealingDoneGraph = ({ start, end, offset, healingBySecond, manaUpdates }: Props) => {
  // TODO: move this to vega-lite window transform
  // e.g. { window: [{op: 'mean', field: 'hps', as: 'hps'}], frame: [-2, 2] }
  const interval = 5;
  const groupedHealingBySecond = groupHealingBySeconds(healingBySecond, interval);
...
export default memo(HealingDoneGraph);
