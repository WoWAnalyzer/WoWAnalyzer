...
abstract class BuffCountGraph extends Analyzer {
  ...
  generateTooltips(): Array<StringFieldDef<Field>> {
    // if we want to go back to "Spell: count" as the tooltip format, it needs to be an extra layer afaik
    return [
      {
        field: 'tooltipName',
        type: 'nominal',
  ...
  generateAreas(): UnitSpec<Field> {
    return {
      data: {
        values: this.buffTrackers,
      },
      mark: {
        type: 'area' as const,
        line: {
      ...
      transform: [
        ...
        {
          calculate: `datum.change.timestamp - ${this.owner.fight.start_time}`,
          as: 'timestamp_shifted',
        },
        // this window op works around the fact that way may get multiple buff
        // changes in a single timestamp
        {
          window: [
            {
              op: 'max',
          ...
          // this frame says "only include entries with the same exact timestamp"
          frame: [0, 0],
          groupby: ['tooltipName'],
          sort: [{ field: 'timestamp_shifted', order: 'ascending' }],
        },
      ],
      encoding: {
        y: {
        ...
        color: {
          ...
          title: 'Spell',
          scale: {
            domain: this.buffTrackers.map(({ tooltipName }) => tooltipName),
            range: this.buffTrackers.map(({ color }) => `rgb(${color})`),
          },
        },
