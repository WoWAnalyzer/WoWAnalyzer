...
const FOOD_MAPPINGS: { [spellId: number]: FoodInfo } = {
  ...
  57138: { itemId: SHOVELTUSK_STEAK, recommendedFood: [FISH_FEAST, FIRECRACKER_SALMON, TENDER_SHOVELTUSK_STEAK] },
  // 35 Spell Power and 40 Stamina
  57096: { itemId: SMOKED_SALMON, recommendedFood: [FISH_FEAST, FIRECRACKER_SALMON, TENDER_SHOVELTUSK_STEAK] },
  ...
  57085: { itemId: GRILLED_SCULPIN, recommendedFood: [FISH_FEAST, MEGA_MAMMOTH_MEAL, POACHED_NORTHERN_SCULPIN] },
  57110: { itemId: MAMMOTH_MEAL, recommendedFood: [FISH_FEAST, MEGA_MAMMOTH_MEAL, POACHED_NORTHERN_SCULPIN] },
  ...
  57098: { itemId: POACHED_NETTLEFISH, recommendedFood: [SPICED_WORM_BURGER, SPICY_BLUE_NETTLEFISH] },
...
const RecommendedFoodList = ({ spellId }: { spellId: number }) => {
  ...
  return <>{foodInfo.recommendedFood.map((higherFoodId: number, index: number) => (<><ItemLink id={higherFoodId} key={index} />&nbsp;</>))}</>;
};
...
class FoodChecker extends Analyzer {
  ...
  get SuggestionText() {
    ...
    if (!this.higherFoodUp && !this.midTierFoodUp) {
      return <>You did not have any food active when starting the fight. Having the right food buff during combat is an easy way to improve performance.</>;
    }
    if (this.midTierFoodUp) {
      return <>
        ...
        {this.recommendedHigherTierFoods && this.recommendedHigherTierFoods.length > 0 && this.activeFoodSpellId &&
          <>
            Instead of using <ItemLink id={FOOD_MAPPINGS[this.activeFoodSpellId].itemId} />, try one of these: <RecommendedFoodList spellId={this.activeFoodSpellId} />
          </>
        }
      </>;
    }
