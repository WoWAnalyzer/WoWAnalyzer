          ...
          <tbody>
            {
              this.wellspringCasts.map((hits, index) => (
                <tr key={index}>
                  ...
                  <td>{ formatMilliseconds(this.wellspringTimestamps[index] - this.owner.fight.start_time) || 0 }</td>
                  <td style={hits < 6 ? {color: 'red' , fontWeight: 'bold'} : {fontWeight: 'bold'}}>{ hits }</td>
                </tr>
              ))
            }
          </tbody>
