...
class ArcanePowerActiveTime extends Analyzer {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.removebuff.by(SELECTED_PLAYER).spell(SPELLS.ARCANE_POWER),
  ...
  onArcanePowerRemoved(event: RemoveBuffEvent) {
    const buffApplied = this.eventHistory.last(
      1,
      undefined,
      Events.applybuff.by(SELECTED_PLAYER).spell(SPELLS.ARCANE_POWER),
    )[0].timestamp;
    ...
    when(this.arcanePowerActiveTimeThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          ...
          <SpellLink id={SPELLS.ARCANE_POWER.id} /> was active. Because a large portion of your
          damage comes from Arcane Power, you should ensure that you are getting the most out of it
          ...
          <SpellLink id={SPELLS.ARCANE_POWER.id} /> when you are at a low risk of being interrupted
          or when the target is vulnerable.
        </>,
      )
