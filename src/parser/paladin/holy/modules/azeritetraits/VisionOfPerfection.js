...
class VisionOfPerfection extends Analyzer {
  ...
  constructor(...args) {
    ...
    this.addEventListener(
      Events.applybuff.by(SELECTED_PLAYER).spell(SPELLS.VISION_OF_PERFECTION_HASTE_BUFF_SELF),
      this.onVisionProc,
    );
    this.addEventListener(
      Events.heal.by(SELECTED_PLAYER).spell(SPELLS.STRIVE_FOR_PERFECTION_HEAL),
      this.onVisionHeal,
    );

    this.hasMajor = this.selectedCombatant.hasMajor(SPELLS.VISION_OF_PERFECTION.traitId);
    if (this.hasMajor) {
      this.majorHaste = calculateAzeriteEffects(
        SPELLS.VISION_OF_PERFECTION_HASTE_CALC.id,
        this.selectedCombatant.neck.itemLevel,
      )[0];
    ...
    if (this.rank > 2) {
      this.minorVersatility = calculateSecondaryStatDefault(
        420,
        45,
        this.selectedCombatant.neck.itemLevel,
      );
  ...
  get visionHasteBuff() {
    const uptime = this.selectedCombatant.getBuffUptime(
      SPELLS.VISION_OF_PERFECTION_HASTE_BUFF_SELF.id,
    );
    ...
    return (
      <StatisticGroup category={STATISTIC_CATEGORY.ITEMS}>
        <ItemStatistic ultrawide size="flexible">
          <div className="pad">
            <label>
              <SpellLink id={SPELLS.STRIVE_FOR_PERFECTION.id} /> - Minor Rank {this.rank}
            </label>
            <div className="value">
              {this.rank > 1 && (
                <>
                  <ItemHealingDone amount={this.minorHealing} />
                  <br />
                </>
              )}
              {this.rank > 2 && (
        ...
        {this.hasMajor && (
          <ItemStatistic ultrawide size="flexible">
            <div className="pad">
              <label>
                <SpellLink id={SPELLS.VISION_OF_PERFECTION.id} /> - Major Rank {this.rank}
              </label>
              <div className="value">
                <EventsIcon /> {this.procs} <small>procs</small>
                <br />
                ...
                <small>uptime {this.extendedBy} seconds</small>
                <br />
                {this.rank > 2 && (
                  <>
