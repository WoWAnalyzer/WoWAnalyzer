...
class VoidTorrent extends Analyzer {
  ...
  suggestions(when: When) {
    ...
    when(this.overcapThreshold).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You lost a total of {formatNumber(this.insanityOvercapped)} insanity by channeling{' '}
          <SpellLink id={SPELLS.VOID_TORRENT_TALENT.id} /> at full insanity. Make sure that you are
          below 40 insanity before channeling to maximize insanity gain. If you are between 40 and
          50 insanity, you should get above 50 and use <SpellLink id={SPELLS.DEVOURING_PLAGUE.id} />{' '}
          before channeling.
        </>,
      )
        .icon(SPELLS.VOID_TORRENT_TALENT.icon)
