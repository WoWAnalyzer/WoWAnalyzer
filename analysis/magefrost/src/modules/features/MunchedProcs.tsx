...
class MunchedProcs extends Analyzer {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.applybuff.by(SELECTED_PLAYER).spell(SPELLS.FINGERS_OF_FROST),
      this.onFingersProc,
    );
    this.addEventListener(
      Events.applybuffstack.by(SELECTED_PLAYER).spell(SPELLS.FINGERS_OF_FROST),
      this.onFingersProc,
    );
  }
  ...
  onIceLanceDamage(event: DamageEvent) {
    ...
    if (this.selectedCombatant.hasBuff(SPELLS.FINGERS_OF_FROST.id, iceLanceCast.timestamp)) {
      this.munchedProcs += 1;
    }
  }
  ...
  suggestions(when: When) {
    when(this.munchedProcsThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You wasted (munched) {this.munchedProcs} <SpellLink id={SPELLS.FINGERS_OF_FROST.id} />{' '}
          procs ({formatPercentage(this.munchedPercent)} of total procs). Because of the way{' '}
          <SpellLink id={SPELLS.FINGERS_OF_FROST.id} /> works, this is sometimes unavoidable (i.e.
          you get a proc while you are using a <SpellLink id={SPELLS.BRAIN_FREEZE.id} /> proc), but
          if you have both a <SpellLink id={SPELLS.BRAIN_FREEZE.id} /> proc and a{' '}
          <SpellLink id={SPELLS.FINGERS_OF_FROST.id} /> proc, you should make sure you use the{' '}
          <SpellLink id={SPELLS.FINGERS_OF_FROST.id} /> procs first before you start casting{' '}
          <SpellLink id={SPELLS.FROSTBOLT.id} /> and <SpellLink id={SPELLS.FLURRY.id} /> to minimize
          the number of wasted/munched procs.
        </>,
      )
  ...
  statistic() {
    return (
      ...
      >
        <BoringSpellValueText spellId={SPELLS.FINGERS_OF_FROST.id}>
