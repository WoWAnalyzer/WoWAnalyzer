...
class Subterfuge extends StealthCasts {
  ...
  get stealthsWithAtleastOneGarrote() {
    ...
    this.stealthSequences.forEach((sequence: CastEvent[]) => {
      const firstGarroteCast = sequence.find(
      ...
      (suggest: SuggestionFactory, actual: number | boolean, recommended: number | boolean) =>
        suggest(
          <>
            ...
            {this.stealthCasts - this.stealthsWithAtleastOneGarrote} time(s). Make sure to
            prioritize snapshotting <SpellLink id={SPELLS.GARROTE.id} /> during{' '}
            <SpellLink id={SPELLS.SUBTERFUGE_BUFF.id} />.
          </>,
        )
          ...
          .actual(
            t({
              ...
              message: `${formatPercentage(
                actual as number,
              )}% of Subterfuges with atleast one Garrote cast`,
            }),
          )
