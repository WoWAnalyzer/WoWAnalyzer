...
class ArcaneMissiles extends Analyzer {
  ...
  onMissilesCast(event: CastEvent) {
    ...
    if (
      spellId === SPELLS.ARCANE_MISSILES.id &&
  ...
  get missilesUtilization() {
    return (
      1 -
      this.castWithoutClearcasting / this.abilityTracker.getAbility(SPELLS.ARCANE_MISSILES.id).casts
    );
  }
  ...
  suggestions(when: When) {
    when(this.arcaneMissileUsageThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You cast <SpellLink id={SPELLS.ARCANE_MISSILES.id} /> improperly{' '}
