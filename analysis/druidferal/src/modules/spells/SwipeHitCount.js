...
class SwipeHitCount extends HitCountAoE {
  ...
  suggestions(when) {
    when(this.hitNoneThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You are using <SpellLink id={SPELLS.SWIPE_CAT.id} /> out of range of any targets. Try to
    ...
    when(this.hitJustOneThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You are using <SpellLink id={SPELLS.SWIPE_CAT.id} /> against a single target. If there's
          only one target in range you'll do more damage by using <SpellLink id={SPELLS.SHRED.id} />{' '}
          instead.
        </>,
      )
        .icon(SPELLS.SWIPE_CAT.icon)
