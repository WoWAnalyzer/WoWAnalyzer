...
const POTENTIAL_SPENDERS = [SPELLS.RIP, SPELLS.FEROCIOUS_BITE, SPELLS.PRIMAL_WRATH_TALENT];
...
const HIT_TYPES_THAT_DONT_CONSUME = [HIT_TYPES.MISS, HIT_TYPES.DODGE, HIT_TYPES.PARRY];
...
class Bloodtalons extends Analyzer {
  ...
  get combinedChart() {
    const items = this.spenders
      ...
      .map((spender) => ({
        ...
        valueTooltip: (
          <>
            <b>{spender.count}</b> charge{spender.count !== 1 ? 's' : ''} used on{' '}
            {spender.spell.name}.
          </>
        ),
      }));
    ...
    items.push({
      ...
      valueTooltip: (
        <>
          <b>{this.wasted}</b> Bloodtalons charge{this.wasted !== 1 ? 's were' : ' was'} wasted.
          <br />
          <ul>
            <li>
              You lost <b>{this.overwritten}</b> by casting 3 different combo point-generating
              abilities within 4 seconds when you already had charges.
            </li>
            <li>
              You lost <b>{this.expired}</b> by allowing {this.expired !== 1 ? 'them' : 'it'} to
              expire.
            </li>
            <li>
              You lost <b>{this.remainAfterFight}</b> by having{' '}
              {this.remainAfterFight !== 1 ? 'them' : 'it'} left over at the fight's end.
            </li>
          </ul>
        </>
      ),
  ...
  constructor(...args) {
    ...
    if (POTENTIAL_SPENDERS.length > CHART_COLOR_SPENDERS.length) {
      throw new Error(
        `Bloodtalons has ${POTENTIAL_SPENDERS.length} potential spenders but only ${CHART_COLOR_SPENDERS.length} chart colors defined.`,
      );
    }
    ...
    this.addEventListener(
      Events.applybuff.to(SELECTED_PLAYER).spell(SPELLS.BLOODTALONS_BUFF),
      this.onApplyBuff,
    );
    this.addEventListener(
      Events.refreshbuff.to(SELECTED_PLAYER).spell(SPELLS.BLOODTALONS_BUFF),
      this.onRefreshBuff,
    );
    this.addEventListener(
      Events.removebuff.to(SELECTED_PLAYER).spell(SPELLS.BLOODTALONS_BUFF),
      this.onRemoveBuff,
    );
    this.addEventListener(
      Events.damage.by(SELECTED_PLAYER).spell(POTENTIAL_SPENDERS),
      this.onDamage,
    );
  ...
  onFightend() {
    ...
    debug &&
      console.log(
        `${this.owner.formatTimestamp(this.owner.fight.end_time, 3)} fight ended with ${
          this.currentStack
        } Bloodtalons charges remaining`,
      );
    ...
    if (
      ...
      console.warn(
        `Not all Bloodtalon charges accounted for. Generated: ${this.generated}, used: ${this.used}, expired: ${this.expired}, remainAfterFight: ${this.remainAfterFight}, overwritten: ${this.overwritten}`,
      );
    }
  }
  ...
  onApplyBuff(event) {
    debug &&
      console.log(
        `${this.owner.formatTimestamp(event.timestamp, 3)} generated 2 Bloodtalons charges`,
      );
    this.generated += 2;
    this.currentStack = 2;
    ...
    debug &&
      console.log(
        `${this.owner.formatTimestamp(
          event.timestamp,
          3,
        )} generated 2 Bloodtalons charges, overwriting ${this.currentStack}`,
      );
  ...
  onRemoveBuff(event) {
    ...
    debug &&
      console.log(
        `${this.owner.formatTimestamp(event.timestamp, 3)} Bloodtalons expired, losing ${
          this.currentStack
        } charges`,
      );
  ...
  onDamage(event) {
    ...
    if (
      this.currentStack === 0 ||
      event.tick ||
      HIT_TYPES_THAT_DONT_CONSUME.includes(event.hitType) ||
  ...
  isAlreadyAccountedForAoE(spellId, timestamp) {
    return (
      this.lastSpender &&
      this.lastSpender.spellId === spellId &&
  ...
  addSpender(spellId) {
    ...
    if (!spender) {
      throw new Error(
        `Tried to add spender ${spellId} which was not listed as a potential spender.`,
      );
    }
    spender.count += 1;
    this.used += 1;
    debug &&
      console.log(
        `${this.owner.formatTimestamp(
          ...
          3,
        )} spend Bloodtalons charge on ${spender.spell.name}`,
      );
  }
  ...
  suggestions(when) {
    when(this.suggestionThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You are overwriting charges of <SpellLink id={SPELLS.BLOODTALONS_TALENT.id} />. Try to use
          both charges before you generate more from casting 3 different combo point-generating
          abilities within 4 seconds that aren't <SpellLink id={SPELLS.SHRED.id} /> or{' '}
          <SpellLink id={SPELLS.BRUTAL_SLASH_TALENT.id} />.
        </>,
      )
  ...
  statistic() {
    return (
      <Statistic position={STATISTIC_ORDER.OPTIONAL(4)} style={{ height: '230px' }}>
        <div className="pad">
          <label>
