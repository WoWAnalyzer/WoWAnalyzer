...
class Netherwalk extends Analyzer {
  ...
  constructor(options: Options) {
    super(options);
    this.active = this.selectedCombatant.hasTalent(DH_TALENTS.NETHERWALK_TALENT.id);
    if (!this.active) {
      return;
    }
  ...
  onNetherwalkCast(event: DamageEvent) {
    if (!this.selectedCombatant.hasBuff(DH_TALENTS.NETHERWALK_TALENT.id)) {
      return;
    }
  ...
  statistic() {
    return (
      ...
      >
        <BoringSpellValueText spellId={DH_TALENTS.NETHERWALK_TALENT.id}>
          <>
