...
class UnempoweredStarfire extends Analyzer {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.applybuff.to(SELECTED_PLAYER).spell(SPELLS.ECLIPSE_LUNAR),
      this.onApplyBuff,
    );
  ...
  suggestions(when: When) {
    when(this.suggestionThresholds).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          You casted {this.badCasts} unempowered and non instant cast{' '}
          <SpellLink id={SPELLS.STARFIRE.id} /> that hit less than {TARGETS_FOR_GOOD_CAST} targets,
          outside the {UNEMPOWERED_CASTS_NEEDED_FOR_EMPOWERMENT} needed to get to{' '}
