...
class QaplaEredunWarOrder extends Analyzer {
  ...
  onBarbedShotCast() {
    ...
    if (killCommandIsOnCooldown) {
      const reductionMs = this.spellUsable.reduceCooldown(
        ...
        QAPLA_KILL_COMMAND_REDUCTION_MS,
      );
  ...
  suggestions(when: When) {
    when(this.cdrEfficiencyKillCommandThreshold).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          ...
          <SpellLink id={SPELLS.KILL_COMMAND_CAST_BM.id} /> has as much cooldown remaining as
          possible to gain the most out of its effect - but you still don't want to be capping on{' '}
        ...
        tooltip={
          <>
            ...
            {this.wastedKCReduction > 0 &&
              `You wasted ${(this.wastedKCReduction / 1000).toFixed(
                2,
              )} seconds of potential cooldown reduction by casting Barbed Shot while Kill Command had less than 4 seconds remaining on its CD.`}
          </>
        }
      >
        <BoringSpellValueText spell={SPELLS.QAPLA_EREDUN_WAR_ORDER_EFFECT}>
          <>
            ...
            <br />
            {formatPercentage(this.effectiveKCReduction / this.totalPossibleCDR)}%{' '}
