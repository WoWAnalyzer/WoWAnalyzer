...
class LashingFlames extends Analyzer {
  ...
  suggestions(when: When) {
    when(this.uptimeThreshold).addSuggestion((suggest, actual, recommended) =>
      suggest(
        <>
          Your <SpellLink id={SPELLS.LASHING_FLAMES_TALENT.id} /> uptime can be improved. As{' '}
          <SpellLink id={SPELLS.LASHING_FLAMES_DEBUFF.id} /> increases the damage of{' '}
          <SpellLink id={SPELLS.FLAME_SHOCK.id} /> on its target,{` `}
          try to maintain 100% uptime for maximum damage increase. To achieve this, you can strike
          the target with <SpellLink id={SPELLS.LAVA_LASH.id} />
          {` `}
        ...
        .actual(
          <>
            <SpellLink id={SPELLS.LASHING_FLAMES_DEBUFF.id} /> was active for{' '}
  ...
  statistic() {
    return (
      <Statistic
        ...
        tooltip={
          <>
            Lashing Flames contributed {formatThousands(this.buffedFlameShockDmg)} total Flame Shock
            damage (
            {formatPercentage(this.owner.getPercentageOfTotalDamageDone(this.buffedFlameShockDmg))}{' '}
            %).
            <br />
          </>
        }
