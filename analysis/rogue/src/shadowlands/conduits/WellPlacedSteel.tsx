...
class WellPlacedSteel extends Analyzer {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.damage.by(SELECTED_PLAYER).spell(SPELLS.DEADLY_POISON),
      this.onDamage,
    );
    this.addEventListener(
      Events.damage.by(SELECTED_PLAYER).spell(SPELLS.DEADLY_POISON_DOT),
      this.onDamage,
    );
    this.addEventListener(
      Events.damage.by(SELECTED_PLAYER).spell(SPELLS.DEADLY_POISON_PROC),
      this.onDamage,
    );
    this.addEventListener(Events.damage.by(SELECTED_PLAYER).spell(SPELLS.ENVENOM), this.onDamage);
    // this.addEventListener(
    //   Events.damage.by(SELECTED_PLAYER).spell(SPELLS.LIQUIFYING_OOZE),
    //   this.onDamage,
    // );
  }
