  ...
  return (
    <SubSection title="">
      <p>
        ...
        is your primary filler spell and will almost always be your most cast spell. It can be used
        on injured raiders or even pre-cast on full health raiders if you know big damage is coming
        soon. Do not spam it unmotivated - you'll run yourself out of mana. You also shouldn't cast
        it on targets that already have a high duration Rejuvenation, as you will clip duration.
        Note that some high-overheal Rejuvs are unavoidable due to heal sniping, but if a large
        proportion of them are, you might be casting too much.
      </p>
      ...
      {info.combatant.hasTalent(SPELLS.SOUL_OF_THE_FOREST_TALENT_RESTORATION) && (
        <>
          <p>
            ...
            <SpellLink id={SPELLS.WILD_GROWTH.id} /> is by far the highest value, but if only a
            single target needs big healing it's acceptable to use{' '}
            <SpellLink id={SPELLS.REJUVENATION.id} /> or <SpellLink id={SPELLS.REGROWTH.id} />.{' '}
            <SpellLink id={SPELLS.CONVOKE_SPIRITS.id} /> can both generate and consume procs - you
            should always use your proc before casting Convoke to avoid overwriting or consuming on
            a bad target. Never let a proc expire.
          </p>
          <p>
