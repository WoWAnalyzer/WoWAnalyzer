...
class MemoryoftheMotherTree extends Analyzer {
  ...
  constructor(options: Options) {
    ...
    this.addEventListener(
      Events.cast.by(SELECTED_PLAYER).spell([SPELLS.REJUVENATION, SPELLS.REGROWTH]),
      this.whatToTrack,
    );
    this.addEventListener(
      Events.applybuff.by(SELECTED_PLAYER).spell([SPELLS.REJUVENATION, SPELLS.REGROWTH]),
      this.handleApplication,
    );
    this.addEventListener(
      Events.refreshbuff.by(SELECTED_PLAYER).spell([SPELLS.REJUVENATION, SPELLS.REGROWTH]),
      this.handleApplication,
    );

    this.addEventListener(
      Events.heal.by(SELECTED_PLAYER).spell([SPELLS.REJUVENATION, SPELLS.REGROWTH]),
      this.handleHeal,
    );

    this.addEventListener(
      Events.removebuff.by(SELECTED_PLAYER).spell([SPELLS.REJUVENATION, SPELLS.REGROWTH]),
      this.handleRemove,
    );
  }
  ...
  wgCast(event: CastEvent) {
    this.totalChances += 1;
    this.procProbabilities.push(0.4);
  }
  ...
  handleHeal(event: HealEvent) {
    if (
      event.ability.guid === SPELLS.REJUVENATION.id &&
      this.rejuvsToTrack.find((e) => e === event.targetID)
    ) {
    ...
    if (
      ...
      this.regrowthToTrack.find((e) => e === event.targetID)
    ) {
